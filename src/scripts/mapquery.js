export default function mapquery(address) {
    ...
    return fetch(searchUrl, {
      ...
      .then(res => (res.ok ? res.json() : new Error('Unable to process location address')))
      .then(json => {
        if (!json || !json.length) return {coords: [14.62, 120.98], display_name: "Manila"};
        return {coords: [parseFloat(json[0].lat), parseFloat(json[0].lon)], display_name: json[0].display_name};
      });
  }